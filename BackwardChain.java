...
public class BackwardChain {
	...
	public BindingList unify(Term t1, Term t2, BindingList bl) {
		// All defaults return null
		if (bl == null) {
			// We can't instantiate a new bl with a null!
			// Is there ever a case when null is passed?
			return null;
		}

		// Instantiate a fresh bindinglist in case we make a new binding
		BindingList freshBL = new BindingList(bl);

		// CASE 1: t1 IS A CONSTANT
		if (t1.c != null) {
			// CASE 1.1: CONSTANT AND CONSTANT
			if (t2.c != null && t1.equals(t2)){
				// t1 and t2 are already bound in bl
				return freshBL;
			}

			// CASE 1.2: CONSTAND AND VARIABLE
			if (t2.v != null) {
				// switch the terms, unify will handle differently
				return unify(t2, t1, freshBL);
			}

			// CASE 1.3: CONSTANT AND FUNCTION
			// cannot be unified
		}

		// CASE 2: t1 IS A VARIABLE
		if (t1.v != null) {
			// CASE 2.1: VARIABLE AND CONSTANT
			if (t2.c != null) {
				if (freshBL.boundValue(t1.v) == null) {
					freshBL.addBinding(t1.v, t2);
					return freshBL;
				}
				if (freshBL.boundValue(t1.v).equals(t2)) {
					return freshBL;
				}
			}

			// CASE 2.2: VARIABLE AND VARIABLE
			if (t2.v != null) {
				if (t1.v.equals(t2.v)) {
					// t1 and t2 are already bound
					return freshBL;
				}
				if (freshBL.boundValue(t1.v) == null) {
					freshBL.addBinding(t1.v, t2);
				...
				if (freshBL.boundValue(t2.v) == null) {
					freshBL.addBinding(t2.v, t1);
					return freshBL;
				}
			}

			// CASE 2.3: VARIABLE AND FUNCTION
			if (t2.f != null) {
				// OCCUR CHECK
		...
		// CASE 3: t1 IS A FUNCTION
		if (t1.f != null) {
			// CASE 3.1: FUNCTION AND CONSTANT
			// cannot be unified

			// CASE 3.2: FUNCTION AND VARIABLE
			if (t2.v != null) {
				return unify(t2, t1, freshBL);
			}
			// CASE 3.3: FUNCTION AND FUNCTION
			if (t2.f != null) {
				return unify(t1.f, t2.f, freshBL);
			}
		}

		return null;
		// In hindsight, it was a lot of work to only have one 'return null' statement
		// debugging is harder since you cannot tell where the code actually failed.
	}
